...
menubar.prototype.moveToNext = function($item) {
	...
	if ($itemUL.is('.root-level')) {
		...
		if (($newItem.attr('aria-haspopup') == 'true') && (this.bChildOpen == true)) {
			var $childMenu = $newItem.children('.dropdown-menu').first();
...
(function($) { 	
	...
	accessibilityLinks($);
	comoMobileBehavior();
	if ($('#menu-main-nav').hasClass('accessible')) {
		var menu1 = new menubar('menu-main-nav', false);
	}
	...
	/*// Prevent child Dropdowns from Closing Parent
	$('.navbar-nav .dropdown .dropdown-menu').on('click', '.dropdown-toggle', function (e) {
		var $this = $(this);
		...
		$parentDropdown.children('.dropdown').not($thisParent).each(function () {
